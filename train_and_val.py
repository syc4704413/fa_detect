...
train_dir = 'H://fa_detect//train_jpg//'
test_dir = 'H://fa_detect//test_jpg//'
train_logs_dir = 'H://fa_detect//train_log'
val_logs_dir = 'H://fa_detect//test_log'
...
def training():
    ...
    train, train_label, val, val_label = fa_detect.input_data.get_files(train_dir, RATIO)
    train_batch, train_label_batch = fa_detect.nput_data.get_batch(train,
                                                  train_label,
                                                  IMG_W,
                                                  IMG_H,
                                                  BATCH_SIZE,
                                                  CAPACITY)
    val_batch, val_label_batch = fa_detect.input_data.get_batch(val,
                                                  val_label,
                                                  IMG_W,
                                                  IMG_H,
                                                  BATCH_SIZE,
    ...
    logits = fa_detect.model.inference(train_batch, BATCH_SIZE, N_CLASSES)
    loss = fa_detect.model.losses(logits, train_label_batch)
    train_op = fa_detect.model.trainning(loss, learning_rate)
    acc = fa_detect.model.evaluation(logits, train_label_batch)
...
def test_one_image():
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as sess:
            ...
            if max_index==0:
                print('This is a close eye with possibility %.6f' %prediction[:, 0])
            else:
                print('This is a open eye with possibility %.6f' %prediction[:, 1])
