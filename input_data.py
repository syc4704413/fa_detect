...
def get_files(file_dir, ratio):
    ...
    for file in os.listdir(file_dir):
        name = file.split(sep='.')
        if 'close' in name[0]:
            close.append(file_dir + file)
            label_close.append(0)
        else:
            if 'open' in name[0]:
                open.append(file_dir + file)
                label_open.append(1)
    print('There are %d close eyes\nThere are %d open eyes' %(len(close), len(open)))

    image_list = np.hstack((close, open))
    label_list = np.hstack((label_close, label_open))
